%{?scl:%scl_package python-expat}
%{!?scl:%global pkg_name %{name}}

Summary: An XML parser library
Name: %{?scl_prefix}python-expat
Version: 2.1.0
Release: 4%{?dist}
...
BuildRequires: autoconf, automake, libtool, check-devel, %{?scl_prefix}build
...
Requires: %{?scl_prefix}expat = %{version}-%{release}

%description devel
The %{?scl_prefix}expat-devel package contains the libraries, include files
and documentation to develop XML applications with expat.

%package static
Summary: expat XML parser static library
Group: Development/Libraries
Requires: %{?scl_prefix}expat-devel%{?_isa} = %{version}-%{release}

%description static
The %{?scl_prefix}expat-static package contains the static version of the
expat library. Install it if you need to link statically with expat.
