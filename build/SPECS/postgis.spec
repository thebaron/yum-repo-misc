...
Name:		%{?scl_prefix}postgis
Version:	2.0.3
Release:	2%{?dist}
License:	GPLv2+
Group:		Applications/Databases
Source0:	http://download.osgeo.org/%{pkg_name}/source/%{pkg_name}-%{version}.tar.gz
Source2:	http://download.osgeo.org/%{pkg_name}/docs/%{pkg_name}-%{version}.pdf
Source4:	filter-requires-perl-Pg.sh
Patch0:		postgis-1.5.1-pgsql9.patch
URL:		http://www.postgis.org
BuildRoot:	%{_tmppath}/%{pkg_name}-%{version}-%{release}-root-%(%{__id_u} -n)

BuildRequires:	%{?scl_prefix}postgresql-devel >= %{pg_version_minimum}
BuildRequires:	proj-devel
BuildRequires:	geos-devel >= 3.1.1
BuildRequires:	byacc, flex, sinjdoc, java, java-devel, ant
BuildRequires:	gtk2-devel, libxml2-devel, gdal-devel >= 1.9.0
Requires:	%{?scl_prefix}postgresql >= %{pg_version_built}, geos >= 3.1.1, proj, gdal >= 1.9.0
...
Group:		Applications/Databases
License:	LGPLv2+
Requires:	%{?scl_prefix}%{pkg_name} = %{version}-%{release}, %{?scl_prefix}postgresql-jdbc
BuildRequires:	ant >= 0:1.6.2, junit >= 0:3.7, %{?scl_prefix}postgresql-jdbc
...
Group:		Applications/Databases
Requires:	%{?scl_prefix}%{pkg_name} = %{version}-%{release}, perl-DBD-Pg
...
%configure --with-gui
#make %{?_smp_mflags} LPATH=`pg_config --pkglibdir` shlib="%{pkg_name}.so"
make LPATH=`pg_config --pkglibdir` shlib="%{pkg_name}.so"

%if %javabuild
export BUILDXML_DIR=%{_builddir}/%{pkg_name}-%{version}/java/jdbc
...
if [ "%{_libdir}" = "/usr/lib64" ] ; then
	mv %{buildroot}%{_datadir}/pgsql/contrib/%{pkg_name}-%{majorversion}/postgis.sql %{buildroot}%{_datadir}/pgsql/contrib/postgis-64.sql
fi

%if %javabuild
install -d %{buildroot}%{_javadir}
install -m 755 java/jdbc/%{pkg_name}-%{version}.jar %{buildroot}%{_javadir}/%{pkg_name}.jar
%if %{gcj_support}
aot-compile-rpm
%endif
strip %{buildroot}/%{_libdir}/gcj/%{pkg_name}/*.jar.so
%endif

%if %utils
install -d %{buildroot}%{_datadir}/%{pkg_name}
install -m 755 utils/*.pl %{buildroot}%{_datadir}/%{pkg_name}
%endif
...
%files
%defattr(-,root,root)
%doc COPYING CREDITS NEWS TODO README.%{pkg_name} doc/html loader/README.* doc/%{pkg_name}.xml doc/ZMSgeoms.txt 
%attr(755,root,root) %{_bindir}/*
%attr(755,root,root) %{_libdir}/pgsql/postgis-*.so
%{_datadir}/pgsql/contrib/*.sql
%{_datadir}/pgsql/contrib/%{pkg_name}-%{majorversion}/*.sql
%{_datadir}/pgsql/contrib/%{pkg_name}-%{majorversion}/postgis_restore.pl
...
%attr(755,root,root) %{_javadir}/%{pkg_name}.jar
%if %{gcj_support}
%dir %{_libdir}/gcj/%{pkg_name}
%{_libdir}/gcj/%{pkg_name}/*.jar.so
%{_libdir}/gcj/%{pkg_name}/*.jar.db
%endif
%endif
...
%{_datadir}/%{pkg_name}/test_estimation.pl
%{_datadir}/%{pkg_name}/profile_intersects.pl
%{_datadir}/%{pkg_name}/test_joinestimation.pl
%{_datadir}/%{pkg_name}/create_undef.pl
%{_datadir}/%{pkg_name}/%{pkg_name}_proc_upgrade.pl
%{_datadir}/%{pkg_name}/%{pkg_name}_restore.pl
%{_datadir}/%{pkg_name}/read_scripts_version.pl
%{_datadir}/%{pkg_name}/test_geography_estimation.pl
%{_datadir}/%{pkg_name}/test_geography_joinestimation.pl
%endif
...
%changelog
* Fri Jul 19 2013 Adam Miller <admiller@redhat.com> - 2.0.3-2
- SCL-ized version of upstream package
